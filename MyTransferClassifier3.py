...
'''
first_input = Input(shape=(1, 62, 128))
first_dense = Dense(128)(first_input)
output_layer = Flatten()(first_dense)
outputs = Dense(85, activation='softmax')(output_layer)

model = Model(inputs=first_input, outputs=outputs)
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['acc'])
model.summary()
model.fit(X_Train_3embed, Y_my_Train, epochs=1, batch_size=32)


####################################### Infer #################################


prediction_probas = model.predict(Xt_Train_3embed) # [
...
for epoch in range(1, 110):
    ...
    model.fit(X_Train_3embed, Y_my_Train, epochs=epoch, batch_size=32)
    prediction_probas = model.predict(Xt_Train_3embed)  # [
    predictions = [np.argmax(pred) for pred in prediction_probas]
    print("f1_score(true_Adad_Yt_my_Test, predictions)")
    print("num epoch:", epoch)
    #print("f1_score-None", f1_score(true_Adad_Yt_my_Test, predictions, average=None))
    print("f1_score-macro", f1_score(true_Adad_Yt_my_Test, predictions, average='macro'))
    print("f1_score-micro", f1_score(true_Adad_Yt_my_Test, predictions, average='micro'))
    print("f1_score-weighted", f1_score(true_Adad_Yt_my_Test, predictions, average='weighted'))
    print("model acc", model.evaluate(X_Train_3embed, Y_my_Train, verbose=0))
