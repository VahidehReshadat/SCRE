...
from keras.layers import Embedding, TimeDistributed, Bidirectional, LSTM, Concatenate, Flatten, Dropout, Dense, Conv1D, \
    GlobalMaxPooling1D
...
class SequenceModel(object):
    ...
    def fit(self, x_train, y_ner_train, y_term_train, y_rel_train,
        ...
        print("Freeze the base model.summary after trainable=false..............")
        model.summary()

        # ***************** Create a feature Extraction model ************************

        my_model_fetr_exr = Model(inputs=model.inputs, outputs=model.layers[-7].output)
        ...
        print("Freeze the Feature extraction model: trainable=false..............")
        ...
        a = Input(batch_shape=(None, None), dtype='int32', name='a')

        b = Input(batch_shape=(None, None, None), dtype='int32', name='b')

        c = Input(shape=(None, 1024), dtype='float32')

        ftrs = my_model_fetr_exr([a, b, c])

        # flt=Flatten()(ftrs)

        # model = Model(inputs=[word_ids, char_ids, elmo_embeddings], outputs=preds)

        # first_dense = Dense(128)(ftrs)
        # output_layer = Dense(83, activation='softmax')(first_dense)
        # outputs = GlobalMaxPooling1D(output_layer)
        # pred_rel = cls_rel(fc_rel)

        fc_rel = TimeDistributed(Dense(128, activation="relu"), name="fc_rel")(ftrs)
        cls_rel = TimeDistributed(Dense(83, activation="sigmoid"), name="cls_rel")
        output_layer = cls_rel(fc_rel)
        outputs = Flatten()(output_layer)

        new_model = Model(inputs=[a, b, c], outputs=outputs)
        new_model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['acc'])
        new_model.fit(X_Train_3embed, Y_my_Train, epochs=2, batch_size=32)
        ...
        # history = new_model.fit((X_Train_3embed, Y_my_Train), epochs=2)
