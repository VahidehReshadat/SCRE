...
class SequenceModel(object):
    ...
    def __init__(self,
                 ...
                 lr=5e-4,
                 rel_pos_bal=3.):
        self.model = load_model("pretrain_models/lstm/dev_40e/weights.h5", custom_objects={'CRF': CRF}, compile=False)
        self.p = ELMoTransformer.load("pretrain_models/lstm/dev_40e/preprocessors.json")
        self.tagger = None
    ...
    def fit(self, x_train, y_ner_train, y_term_train, y_rel_train,
            x_valid, y_ner_valid, y_term_valid, y_rel_valid, X_Train_3embed, Y_my_Train,
            embeddings_file, elmo_options_file, elmo_weights_file, p, rel_pos_bal, lr,
            steps_per_epoch=None, epochs=1, batch_size=32, verbose=1, callbacks=None, shuffle=True):

        # ***************** Instantiate a base model with pretraining weights************************

        print("elmo")
        # p = ELMoTransformer(elmo_options_file, elmo_weights_file)
        # p.fit(x_train, y_ner_train, y_term_train)
        # modelp = ELMoTransformer.load("pretrain_models/lstm/dev_40e/preprocessors.json")
        print("glove")
        ...
        # model.compile(loss=[crf_loss, crf_loss, weighted_binary_crossentropy(rel_pos_bal)], optimizer=Adam(lr=lr))

        # ***************** Freeze the base model ************************

        model.trainable = False
        print("model asli summary 2 bade trainable=false..............")
        model.summary()

        # ***************** Create a feature Extraction model ************************

        my_model_fetr_exr = Model(inputs=model.inputs, outputs=model.layers[-5].output)

        my_model_fetr_exr.trainable = False
        # ftrs_from_old = my_model_fetr_exr([char_ids, word_ids, elmo_embeddings])
        print(len(my_model_fetr_exr.layers))
        print("model asli summary 2 bade trainable=false..............")
        my_model_fetr_exr.summary()

        print("Shape vorodye model ..............")
        print(my_model_fetr_exr.input_shape)
        # print(my_model_fetr_exr.inputs.shape)
        ftrs_from_old = my_model_fetr_exr.predict(X_Train_3embed[0])
        # ftrs_from_old1 = my_model_fetr_exr(X_Train_3emb[0])

        # print(X_Train_3emb[0].shape)

        print("IIIIIIIIIIIIIIIIIIIIIIIIIftrs_from_old")

        print(ftrs_from_old)
        print("len(ftrs_from_oldddddddddddddddddddddddddddddddddddddddddddddddddddddddd)")
        # (ftrs_from_old[0][0][0])
        # print(ftrs_from_old[0].shape)
        # print(type(ftrs_from_old))
        # print(ftrs_from_old1.shape)

        print(ftrs_from_old)

        # print(len(ftrs_from_old[]))

        X_Train_3embeding = list()

        for i in X_Train_3embed:
            ftrs_from_old1 = my_model_fetr_exr.predict(i)
            # a = np.array.flatten(ftrs_from_old1)
            X_Train_3embeding.append(ftrs_from_old1)

        dump(X_Train_3embeding, open('X_Test_3embeding.pkl', 'wb'))
        # dump(X_Train_3embeding, open('X_Train_3embeding.pkl', 'wb'))

        print("X_Train_3embedinggggggggggggggggggggggggggggggggggggggggggggggggggggg[0]")

        print(X_Train_3embeding[0].shape)
        print(X_Train_3embeding[1].shape)

        print(len(X_Train_3embeding))
        print(len(X_Train_3embeding[0]))

        print(type(X_Train_3embeding[0]))
        print(type(X_Train_3embeding))

        ''' 

        #***************** Create NEw model ************************

        #word_ids = Input(batch_shape=(None, None), dtype='int32', name='word_ids') #1-tensor ke 2-bodi ast v abade bodha moshakhas niss
        #char_ids = Input(batch_shape=(None, None, None), dtype='int32', name='char_ids') #Avalin layer model ghadim
        #elmo_embeddings = Input(shape=(None, 1024), dtype='float32')

        my_word_ids = Input(shape=(None, None), dtype='int32', name='word_ids') #1-tensor ke 2-bodi ast v abade bodha moshakhas niss
        my_char_ids = Input(shape=(None, None, None), dtype='int32', name='char_ids') #Avalin layer model ghadim
        my_elmo_embeddings = Input(shape=(1024, ), dtype='float32')


        #print("lennnnnnn [char_ids, word_ids, elmo_embeddings]")
       # print(len([char_ids, word_ids, elmo_embeddings]))
       # print(char_ids.shape)

        #myinpt1=Input(shape=(None, None))
       # myinpt2=Input(shape=(None, None, None))
        #myinpt3=Input(shape=(None, None, 1024))


        #my_inputs=Input(shape=[word_ids, char_ids, elmo_embeddings])

        ftrs_from_old = my_model_fetr_exr([my_char_ids, my_word_ids, my_elmo_embeddings])
        #frts = my_nw_model.predict(preproc)

        #ftrs_from_old = my_model_fetr_exr.predict([myinpt1, myinpt2, myinpt3])

        #ftrs_from_old = my_model_fetr_exr.predict(X_Train_3emb[0])


        print(ftrs_from_old.shape)
        print(type(ftrs_from_old))
        ...
        #ftrs_from_old = my_model_fetr_exr(X_Train_3emb)


        #Flatten kardane ftr_from_pld az 3bod -> (1,13,128) v (1,20, 128)  -> bode vorodieye modele TestRe ... Global Average PIOOOling


        print("After extractingggg Ftr_from_old")

        flt_ftrs_from_old = layers.Flatten(ftrs_from_old)

        print("After Flattennnnnnnnnnnnnnnnn Ftr_from_old")

        print(flt_ftrs_from_old)

        print(flt_ftrs_from_old.shape)

        my_fc_rel = Dense(64, activation="relu")(ftrs_from_old)

        my_cls_rel = TimeDistributed(Dense(83, activation="sigmoid"), name="my_cls_rel")

        my_pred_rel = my_cls_rel(my_fc_rel)
        preds2 = [my_pred_rel]

        my_new_model = Model(inputs=my_model_fetr_exr.inputs, outputs=preds2)  # creae model
        my_new_model.summary()
        print(len(my_new_model.layers))
'''

        """ ######Train modele_base ke be nazaram lazem nist?!!!! be nazaram age fine-tune bood lazem mishod shayad
...
class MultiLayerLSTM(object):
    ...
    def build(self):
        ...
        # for lyrs in model.layers[:]:   #Freeze all layesr except last one
...
class Tagger(object):
    ...
    def analyze(self, text):
        ...
        embed_inpt = preds[3]  ### embeded input  ->[word, char, elmo]
        print("preds 0-2")
        preds = preds[0:3]
        print(embed_inpt)
