...
if __name__ == "__main__":
    ...
    X_Train_3emb = pd.read_pickle("lis_X_all_Test_Ds1_Small.pkl")

    Y_Train_labels_list = pd.read_pickle("lis_Y_all_Test_Ds1_small.pkl")
    ...
    history = model.fit(x_train, y_ner_train, y_term_train, y_rel_train,
                        x_test, y_ner_test, y_term_test, y_rel_test, X_Train_3emb, Y_my_Train,
                        ...
                        p=ELMoTransformer.load("pretrain_models/lstm/dev_40e/preprocessors.json"),
                        rel_pos_bal=3.,
                        lr=5e-4,
                        steps_per_epoch=18,
                        epochs=20,
...
'''
    ...
    analysis, embed_input = model.analyze(online3)
    print(analysis)
    print("shapeeeeeeeeeee_embed_inpu")
    #print(np.embed_input)
    #print(embed_input.shape)
    print(analysis)

    print(len(embed_input))

    # print(X_Train_3emb.shape)
    print(embed_input[0].shape)
    print(embed_input[0].shape)
    print(embed_input[0].shape)
